...
{
	std::cout << "SAMPLE APPLICATUON FOR OpenCV\n";
	std::cout << "\n";
...
int ImageHandler(const int display_scale, const int show_mode, const int display_mode, ImageState* image_state)
{
	...
	if (camera_status) {
		...
		if (is_color_exists) {
			// color image
			cv::Mat mat_s0_image(image_state->isc_image_Info.frame_data[fd_inex].color.height, image_state->isc_image_Info.frame_data[fd_inex].color.width, CV_8UC3, image_state->isc_image_Info.frame_data[fd_inex].color.image);
			...
			// base image
			cv::Mat mat_s0_image(image_state->isc_image_Info.frame_data[fd_inex].p1.height, image_state->isc_image_Info.frame_data[fd_inex].p1.width, CV_8U, image_state->isc_image_Info.frame_data[fd_inex].p1.image);
	...
	if (display_mode == 1) {
		if (camera_status && data_proc_status) {
			...
			if (mat_s0_image_scale_flip.empty()) {
				return 0;
			}
			...
			if (is_color_exists) {
				// color image
				cv::Mat mat_s0_image(image_state->isc_data_proc_result_data.isc_image_info.frame_data[fd_inex].color.height,
				...
				// base image
				cv::Mat mat_s0_image(image_state->isc_data_proc_result_data.isc_image_info.frame_data[fd_inex].p1.height,
