...
void BlockMatching::initialize(int imghgt, int imgwdt)
{
	// バッファーを確保する
	// ブロック視差値（サブピクセル精度：浮動小数）の配列（視差ブロックごと）
...
void BlockMatching::setUseOpenCLForBlockMatching(int usecl)
{
...
void BlockMatching::setMatchingParameter(int imghgt, int imgwdt, int depth,
...
void BlockMatching::setBackMatchingParameter(int enb, int bkevlwdt, int bkevlrng, int bkvldrt, int bkzrrt)
{
...
void BlockMatching::matching(unsigned char* prgtimg, unsigned char* plftimg)
{
...
void BlockMatching::getBlockDisparity(int *pblkhgt, int *pblkwdt, int *pmtchgt, int *pmtcwdt,
...
void BlockMatching::getDisparity(int imghgt, int imgwdt, unsigned char *pdspimg, float *ppxldsp)
{
	// 視差ブロックの高さ
...
void BlockMatching::executeMatching(int imghgt, int imgwdt, int depth, 
	...
	float * pblkdsp, int *pblkcrst)
{
	...
	// 視差を取得する
	getMatchingDisparity(imghgt, imgwdt, depth, crstthr, crstofs, bgtmax,
...
void BlockMatching::getMatchingDisparity(int imghgt, int imgwdt, int depth, int crstthr, int crstofs, int bgtmax,
...
{
	if (numOfBands < 2) {
		getWholeDisparity(imghgt, imgwdt, depth, crstthr, crstofs, bgtmax,
			stphgt, stpwdt, blkhgt, blkwdt, imghgtblk, imgwdtblk,
			pimgref, pimgcmp, pblkdsp, pblkbkdsp, pblkcrst);
	}
	else {
		getBandDisparity(imghgt, imgwdt, depth, crstthr, crstofs, bgtmax,
			stphgt, stpwdt, blkhgt, blkwdt, imghgtblk, imgwdtblk,
			pimgref, pimgcmp, pblkdsp, pblkbkdsp, pblkcrst);
	}
...
/// <param name="pblkbkdsp">バックマッチングの視差値(OUT)</param>
/// <param name="pblkcrst">ブロックコントラスト(OUT)</param>
void BlockMatching::getWholeDisparity(int imghgt, int imgwdt, int depth, int crstthr, int crstofs, int bgtmax,
	int stphgt, int stpwdt, int blkhgt, int blkwdt, int imghgtblk, int imgwdtblk,
	unsigned char *pimgref, unsigned char *pimgcmp, float *pblkdsp, float *pblkbkdsp, int *pblkcrst)
{

	if (pblkbkdsp == NULL) {
		getDisparityInBand(imghgt, imgwdt, depth, crstthr, crstofs, bgtmax,
			stphgt, stpwdt, blkhgt, blkwdt, imghgtblk, imgwdtblk,
			pimgref, pimgcmp, pblkdsp, pblkcrst, 0, imghgt);
	}
	else {
		getBothDisparityInBand(imghgt, imgwdt, depth, crstthr, crstofs, bgtmax,
			stphgt, stpwdt, blkhgt, blkwdt, imghgtblk, imgwdtblk,
...
void BlockMatching::getDisparityInBand(int imghgt, int imgwdt, int depth, int crstthr, int crstofs, int bgtmax,
	int stphgt, int stpwdt, int blkhgt, int blkwdt, int imghgtblk, int imgwdtblk,
	...
	int jstart, int jend)
{
	...
	for (int jpx = jstart; jpx < jend && jpx <= (imghgt - blkhgt); jpx++) {
		for (int ipx = 0; ipx <= (imgwdt - depth - blkwdt); ipx++) {
			// SSDにより視差値を求める
			getDisparityBySSD(ipx, jpx, imghgt, imgwdt, depth, crstthr, crstofs, bgtmax,
				stphgt, stpwdt, blkhgt, blkwdt, imghgtblk, imgwdtblk,
...
/// <param name="pblkdsp">マッチングの視差値(OUT)</param>
/// <param name="pblkcrst">ブロックコントラスト(OUT)</param>
void BlockMatching::getDisparityBySSD(int x, int y, int imghgt, int imgwdt, int depth,
...
/// <param name="jstart">バンド開始ブロック位置(IN)</param>
/// <param name="jend">バンド終了ブロック位置(IN)</param>
void BlockMatching::getBothDisparityInBand(int imghgt, int imgwdt, int depth, int crstthr, int crstofs, int bgtmax,
	int stphgt, int stpwdt, int blkhgt, int blkwdt, int imghgtblk, int imgwdtblk,
	unsigned char *pimgref, unsigned char *pimgcmp, float *pblkdsp, float *pblkbkdsp, int *pblkcrst,
	int jstart, int jend)
{
	...
	for (int jpx = jstart; jpx < jend && jpx <= (imghgt - blkhgt); jpx++) {
		for (int ipx = 0; ipx <= (imgwdt - blkwdt); ipx++) {
			// SSDにより視差値を求める
			getBothDisparityBySSD(ipx, jpx, imghgt, imgwdt, depth, crstthr, crstofs, bgtmax,
				stphgt, stpwdt, blkhgt, blkwdt, imghgtblk, imgwdtblk,
				pimgref, pimgcmp, pblkdsp, pblkbkdsp, pblkcrst);
		}
	}
...
/// <param name="pblkbkdsp">バックマッチングの視差値(OUT)</param>
/// <param name="pblkcrst">ブロックコントラスト(OUT)</param>
void BlockMatching::getBothDisparityBySSD(int x, int y, int imghgt, int imgwdt, int depth,
	int crstthr, int crstofs, int bgtmax,
	int stphgt, int stpwdt, int blkhgt, int blkwdt, int imghgtblk, int imgwdtblk,
	unsigned char *pimgref, unsigned char *pimgcmp,
	float * pblkdsp, float * pblkbkdsp, int *pblkcrst)
{
	...
	int bk_crst = 0;
	if (bk_Lmax >= bgtmax) {
		bk_crst = ((bk_Lmax - bk_Lmin) * 1000 - crstofs) * blkcnt / bk_sumr;
	}
...
void BlockMatching::blendBothMatchingDisparity(int imghgt, int imgwdt, int imghgtblk, int imgwdtblk, 
...
void BlockMatching::spreadDisparityImage(int imghgt, int imgwdt, int depth, int shdwdt,
...
/// <param name="pblkdsp">ブロックごとの視差値(OUT)</param>
/// <param name="pblkcrst">ブロックコントラスト(OUT)</param>
void BlockMatching::executeMatchingOpenCL(int imghgt, int imgwdt, int depth,
	unsigned char *pimgref, unsigned char *pimgcmp,
	float * pblkdsp, int *pblkcrst)
{
	...
	if (enableBackMatching == 0) {
		...
		// SSDにより視差値を求める
		getDisparityBySSDOpenCL(imghgt, imgwdt, depth, crstthr, crstofs, bgtmax,
			stphgt, stpwdt, blkhgt, blkwdt, imghgtblk, imgwdtblk,
		...
		// 両方向マッチングによる視差を取得する
		getBothDisparityBySSDOpenCL(imghgt, imgwdt, depth, crstthr, crstofs, bgtmax,
			stphgt, stpwdt, blkhgt, blkwdt, imghgtblk, imgwdtblk,
...
static char* kernelGetDisparityBySSD = (char*)"__kernel void kernelGetDisparityBySSD(\n\
	int imghgt, int imgwdt, int depth, int crstthr, int crstofs, int bgtmax,\n\
...
void BlockMatching::getDisparityBySSDOpenCL(int imghgt, int imgwdt, int depth, int crstthr, int crstofs, int bgtmax,
	int stphgt, int stpwdt, int blkhgt, int blkwdt, int imghgtblk, int imgwdtblk,
...
/// <param name="blkcrst">ブロックコントラスト(OUT)</param>
void BlockMatching::getBothDisparityBySSDOpenCL(int imghgt, int imgwdt, int depth, int crstthr, int crstofs, int bgtmax,
	int stphgt, int stpwdt, int blkhgt, int blkwdt, int imghgtblk, int imgwdtblk,
...
UINT  BlockMatching::matchingBandThread(LPVOID parg)
{
	...
	while (1) {
		...
		if (pBand->pblkbkdsp == NULL) {
			getDisparityInBand(pBand->imghgt, pBand->imgwdt, pBand->depth, pBand->crstthr, pBand->crstofs, pBand->bgtmax,
			...
			getBothDisparityInBand(pBand->imghgt, pBand->imgwdt, pBand->depth, pBand->crstthr, pBand->crstofs, pBand->bgtmax,
				pBand->stphgt, pBand->stpwdt, pBand->blkhgt, pBand->blkwdt, pBand->imghgtblk, pBand->imgwdtblk,
...
/// ブロックマッチングスレッドを生成する
/// </summary>
void BlockMatching::createMatchingThread()
{
...
/// マッチングスレッドを破棄する
/// </summary>
void BlockMatching::deleteMatchingThread()
{
	// バンド数が2以上の場合
	if (numOfBands > 1) {
		for (int i = 0; i < numOfBands; i++) {
...
/// <param name="pblkbkdsp">バックマッチングの視差値(OUT)</param>
/// <param name="pblkcrst">ブロックコントラスト(OUT)</param>
void BlockMatching::getBandDisparity(int imghgt, int imgwdt, int depth, int crstthr, int crstofs, int bgtmax,
	int stphgt, int stpwdt, int blkhgt, int blkwdt, int imghgtblk, int imgwdtblk,
	unsigned char *pimgref, unsigned char *pimgcmp, float *pblkdsp, float *pblkbkdsp, int *pblkcrst)
{
