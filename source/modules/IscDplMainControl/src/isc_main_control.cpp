 ...
 * @retval other 失敗
 */
int IscMainControl::GetFileInformation(wchar_t* play_file_name, IscRawFileHeader* raw_file_header, IscPlayFileInformation* play_file_information)
{
    if (isc_main_control_impl_ == nullptr) {
        return ISCDPL_E_INVALID_HANDLE;
    }
    ...
    int ret = isc_main_control_impl_->GetFileInformation(play_file_name, raw_file_header, play_file_information);
    if (ret != DPC_E_OK) {
        return ret;
    }
 ...
 * @retval other 失敗
 */
int IscMainControl::SetReadFrameNumber(const __int64 frame_number)
{
    if (isc_main_control_impl_ == nullptr) {
        return ISCDPL_E_INVALID_HANDLE;
    }

    int ret = isc_main_control_impl_->SetReadFrameNumber(frame_number);
    if (ret != DPC_E_OK) {
        return ret;
    }
 ...
 * @retval other 失敗
 */
int IscMainControl::GetFileReadStatus(__int64* frame_number, IscFileReadStatus* file_read_status)
{
    if (isc_main_control_impl_ == nullptr) {
        return ISCDPL_E_INVALID_HANDLE;
    }

    int ret = isc_main_control_impl_->GetFileReadStatus(frame_number, file_read_status);
    if (ret != DPC_E_OK) {
        return ret;
    }
